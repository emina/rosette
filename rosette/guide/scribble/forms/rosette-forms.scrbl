...
@(define rosette-eval (rosette-evaluator) #;(rosette-log-evaluator (logfile root "rosette-forms-log")))
...
@defform[(verify expr)]{
 ...
 @racket[(spec-assumes #, @var{P})] ∧ @racket[(spec-asserts #, @var{P})] ∧
 @racket[(spec-assumes #, @var{Q})] ∧ ¬ @racket[(spec-asserts #, @var{Q})],
 ...
 @racket[(vc)] unchanged after the query returns.
  ...
  (vc)
  (code:line "This query forces a to be false:")
  (verify (assert a)) 
...
@defform*[((synthesize input expr)
 ...
 from the symbolic constants @var{H} to concrete values that 
 ...
 where @var{pre}(@var{H}, @var{I}) is @racket[(spec-assumes #, @var{P})] ∧
 @racket[(spec-asserts #, @var{P})] ∧ @racket[(spec-assumes #, @var{Q})],
 @var{post}(@var{H}, @var{I}) is @racket[(spec-asserts #, @var{Q})],
 ...
 be false on all inputs @var{I}. The formulas @var{pre}(@var{H}, @var{I}) and
...
@defform[(optimize
  ...
  before the invocation of @racket[optimize] and during the evaluation of @racket[min-expr],
  @racket[max-expr], and @racket[expr].  
  ...
  to the cost terms provided in the @racket[min-expr] and @racket[max-expr] lists.  Specifically, these
  ...
  evaluating @racket[min-expr],
  @racket[max-expr], and @racket[expr] are removed from the global @tech["assertion store"] once
  ...
  (define-symbolic x y integer?)
  (assert (< x 2))
  (code:line (vc)   (code:comment "assertion added to the store")) 
  (define sol
    (optimize #:maximize (list (+ x y))
              #:guarantee (assert (< (- y x) 1))))
  (code:line (vc)   (code:comment "assertion store same as before"))
