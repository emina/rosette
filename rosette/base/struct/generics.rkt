       ...
       (with-syntax ([id? (format-id #'id "~a?" #'id #:source #'id)]
         ...
         (syntax/loc stx 
           (begin
             ...
             (set! id? (lift 'id? id? receiver))
             (set! support-name (lift 'foo support-name receiver))
             (set! method-name (lift 'method-name method-name receiver)) ...))))]))
...
(define-syntax-rule (lift sym proc receiver)
  (let ([proc proc])
    (procedure-rename
