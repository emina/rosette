...
(define (make-z3 #:path [path #f])
  (define real-z3-path (super/find-solver "z3" z3-path path))
...
(struct z3 super/solver ()
  ...
  [
   ...
   (define (solver-minimize self nums)
     (super/set-solver-mins! self (append (super/solver-mins self) (numeric-terms nums 'solver-minimize))))
   
   (define (solver-maximize self nums)
     (super/set-solver-maxs! self (append (super/solver-maxs self) (numeric-terms nums 'solver-maximize))))
   
   (define (solver-clear self) 
     (super/solver-clear-stacks! self)
     (super/solver-clear-env! self)
     (server-write (super/solver-server self) (reset))
     (set-default-options (super/solver-server self)))
   
   (define (solver-shutdown self)
     (solver-clear self)
     (server-shutdown (super/solver-server self)))
   ...
   (define (solver-debug self)
     ...
     (cond [(ormap false? asserts) (unsat (list #f))]
           [else (super/solver-clear-env! self)
                 (server-write (super/solver-server self) (reset))
                 (set-core-options (super/solver-server self))
                 (server-write
                  server
                  (begin (encode-for-proof (super/solver-env self) asserts)
                         (check-sat)))
                 (super/read-solution server (super/solver-env self) #:unsat-core? #t)]))])
