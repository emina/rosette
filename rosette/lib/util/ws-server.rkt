...
(require racket/require
         ...
         (multi-in web-server/http (request-structs response response-structs))
         web-server/dispatchers/dispatch
         net/url
         net/rfc6455/dispatcher
         (except-in net/rfc6455 ws-serve))

(provide ws-serve (all-from-out net/rfc6455))

(define (transpose xss) (apply map list xss))

(define ((error-handler conn req) e)
...
(define ((guard-dispatcher d) conn req)
  (with-handlers ([exn:dispatcher? (error-handler conn req)])
    (d conn req)))
