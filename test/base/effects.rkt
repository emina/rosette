...
(define minimized
  (test-suite+ 
   ...
   (let* ([z (@box 10)]
     ...
     (let-values ([(o s) (speculate #t (begin (z-set! 2) (@unbox z)))])
       (check-equal? o 2)
...
(define tests
  (test-suite+
   ...
   (let-values ([(o s) (speculate #t 1)])
     (check-equal? o 1))
   
   (let-values ([(o s) (speculate #t (+ x y))])
     (check-equal? o (+ x y)))
   
   (let-values ([(o s) (speculate #t (begin (set! x 3) x))])
     (check-equal? o 3)
     (check-equal? x 0))
   
   (let-values ([(o s) (speculate #t (begin (set! x 5) x))])
     (check-equal? o 5)
     (check-equal? x 0)
     (s +)
     (check-equal? x 5)
   ...
   (let-values ([(o s) (speculate #t (let ([tmp x])
                                       (set! x y)
                                       (set! y tmp)))])
     (check-true (void? o))
     (check-equal? x 35)
   ...
   (let ([z (@box 10)])
     (let-values ([(o s) (speculate #t (begin (@set-box! z 2) (@unbox z)))])
       (check-equal? o 2)
...
(define struct-tests
  (test-suite+
   ...
   (let-values ([(o s) (speculate #t (begin (set-pos-x! p 3) (pos-x p)))])
     (check-equal? o 3)
   ...
   (set-pos-x! p 0)
   (let-values ([(o s) (speculate #t (let ([r p]) (set-pos-x! r 3) (pos-x r)))]) ; aliasing
     (check-equal? o 3)
     (check-equal? (pos-x p) 0)
     (s +)
     (check-equal? (pos-x p) 3)
   ...
   (let ([z (@box (pos 1 5))])
     (let-values ([(o-outer s-outer) 
                   (speculate #t (let ([q p][proc set-pos-x!])
                                   (let-values ([(o-inner s-inner)
                                                 (speculate #t (begin (set-pos-x! q 4)
                                                                      (proc (@unbox z) 2)
                                                                      (@set-box! z q)))])
...
(define vector-tests
  (test-suite+
   ...
   (let-values ([(o s) (speculate #t (begin (@vector-set! v 2 3) (@vector-ref v 2)))])
     (check-equal? o 3)
   ...
   (@vector-set! v 2 0)
   (let-values ([(o s) (speculate #t (let ([r v]) (@vector-set! r 1 3) (@vector-ref r 1)))]) ; aliasing
     (check-equal? o 3)
     (check-equal? v (make-vector 3 0))
     (s +)
   ...
   (let ([z (@box (@vector 1 3 5 6))])
     (let-values ([(o-outer s-outer) 
                   (speculate #t (let ([q v][proc @vector-set!])
                                   (let-values ([(o-inner s-inner)
                                                 (speculate #t (begin (@vector-set! q 0 4)
                                                                      (proc (@unbox z) 0 2)
                                                                      (@set-box! z q)))])
                                     (list o-inner s-inner))))])
       (let ([o-inner (first o-outer)] [s-inner (second o-outer)])
