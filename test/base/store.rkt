...
(define (check-box-0)
  (define x (@box 1))
  (define y (@box 2))
  (check-match (with-store 1) (ans 1 '()))
  (check-match (with-store (+ (@unbox x) (@unbox y))) (ans 3 '()))
        ...
        (let ([tmp (@unbox x)])
          (with-store
              (begin (@set-box! x (@unbox y))
                     ...
                     (@set-box! y (@unbox z))
                     (@set-box! z tmp))))))
  (check-match r (ans (ans (? void?) (? list?)) (list)))
  (match-define (ans (ans _ s) _) r)
  (check-equal? (@unbox x) 1)
  (check-equal? (@unbox y) 2)
...
(define (check-box-5)
  ...
  (check-match r (ans (ans (? void?) (? list?)) (? list ?)))
  (match-define (ans (ans _ s1) s2) r)
  (check-equal? (@unbox x) 1)
  (check-equal? (@unbox y) 2)
  (check-equal? (@unbox z) 3)
...
(define (check-vector-0)
  ...
  (check-match r1 (ans (? void?) (? list?)))
  (match-define (ans _ s1) r1)
  ...
  (check-match r2 (ans (? void?) (? list?)))
  (match-define (ans _ s2) r2)
  (check-equal? v (@vector 1 5 3))
  ...
  (check-match r3 (ans (? void?) (? list?)))
  (match-define (ans _ s3) r3)
  (check-equal? v (@vector 5 5 4))
...
(define (check-vector-2)
  ...
  (match-define (ans (ans (? void?) s2i) s2o) r2)
  (check-eq? (@unbox z) v)
  (check-equal? v (@vector 1 2 3))
  (check-equal? w (@vector 4 5))
...
(define (check-merge-0)
  (define v (@vector 1 2 3))
  (match-define (ans 5 s1) (with-store (+ (@vector-ref v 1) (@vector-ref v 2))))
  (match-define (ans 1 s2) (with-store (@vector-ref v 0)))
  (check-equal? s1 null)
  (check-equal? s2 null)
  (check-equal? v (@vector 1 2 3))
  (define-symbolic b @boolean?)
...
(define (check-merge-2)
  (define v (@vector 1 2 3))
  (define-symbolic b @boolean?)
  (match-define (ans (? void?) s1t) (with-store (begin (@vector-set! v 0 4) (@vector-set! v 1 5) (@vector-set! v 2 6))))
  (match-define (ans (? void?) s1f) (with-store (begin (@vector-set! v 0 -4) (@vector-set! v 1 -5) (@vector-set! v 2 -6))))
  (merge-stores! (list b (! b)) (list s1t s1f))
  (check-equal? v (@vector (merge b 4 -4) (merge b 5 -5) (merge b 6 -6)))
  (@vector-copy! v 0 (vector 1 2 3))
  (match-define (ans (? void?) s2t) (with-store (begin (@vector-set! v 0 4) (@vector-set! v 2 6))))
  (match-define (ans (? void?) s2f) (with-store (begin (@vector-set! v 1 -5))))
  (merge-stores! (list b (! b)) (list s2t s2f))
  (check-equal? v (@vector (merge b 4 1) (merge b 2 -5) (merge b 6 3)))
  (@vector-copy! v 0 (vector 1 2 3))
  (match-define (ans (? void?) s3t) (with-store (begin (@vector-set! v 0 4) (@vector-set! v 1 5))))
  (match-define (ans (? void?) s3f) (with-store (begin (@vector-set! v 1 -5) (@vector-set! v 2 -6))))
...
(define (check-merge-3)
  ...
  (match-define (ans (? void?) s1)
    (with-store
        (begin (@vector-set! v 0 11)
               (@vector-set! v 1 12)
               (@vector-set! v 2 13)
               (@vector-set! v 3 14))))
  (match-define (ans (? void?) s2)
    (with-store
        (begin (@vector-set! v 1 22)
               (@vector-set! v 3 24)
               (@vector-set! v 4 25)
               (@vector-set! v 5 26))))
 (match-define (ans (? void?) s3)
    (with-store
