...
(define (check-lifted-extract)
  ...
  (check-state (@extract i 3 (phi (cons a x) (cons b 3) (cons c y) (cons d '()))) 
               ...
               (list  (@<= 3 i) (=> (|| a c) (@< i 4)) (|| a c)))
  (check-state (@extract i 2 (phi (cons a x) (cons b 3) (cons c y) (cons d '()))) 
               (phi (cons (&& (|| a c) (@= i 3)) (@extract 3 2 (phi (cons a x)  (cons c y))))
                    (cons (&& (|| a c) (@= i 2)) (@extract 2 2 (phi (cons a x)  (cons c y)))))
               (list  (@<= 2 i) (=> (|| a c) (@< i 4)) (|| a c)))
  ...
  (check-state (@extract 1 j (phi (cons a x) (cons b 3) (cons c y) (cons d '())))  
               (phi (cons (&& (|| a c) (@= j 0)) (@extract 1 0 (phi (cons a x)  (cons c y)))) 
                    (cons (&& (|| a c) (@= j 1)) (@extract 1 1 (phi (cons a x)  (cons c y))))) 
               (list (@<= j 1) (@<= 0 j) (|| a c)))
